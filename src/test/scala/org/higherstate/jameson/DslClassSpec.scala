...
class DslClassSpec extends WordSpec with MustMatchers  {
  ...
  "Simple class parsing" should {
    ...
    "parse class with nested map" in {
      as[MapChild]("map" -> #^("value" -> as[Any])).parse("""{"map":{"value":3,"value2":false}}""") mustEqual (Success(MapChild(Map("value" -> 3))))
    }
    ...
    "parse class with an internal function piping" in {
      as[Child3]("int" -> "tInt" -> as[Int] |> (_ * 2), "tBool" -> ?(false)).parse("""{"int":12}""")  mustEqual (Success(Child3(24, false)))
    }
  ...
  "Conditional class parsing" should {
    "Handle matching on string values" in {
      /("type", "int" -> as[Child1]("value" -> "tInt" -> as[Int]), "bool" -> as[Child2]("value" -> "tBool" -> as[Boolean])).parse("""{"type":"bool","value":false}""") mustEqual (Success(Child2(false)))
    }

    "Handle matching on the class name" in {
      /("class", as[Child1]("value" -> "tInt" -> as[Int]),as[Child2]("value" -> "tBool" -> as[Boolean]),as[Child3]("value1" -> "tBool" -> as[Boolean], "value2" -> "tInt" -> as[Int]))
  ...
  "Either class parsing" should {
    ...
    "Handle a correct right parse" in {
      ^(as[Child1], as[Child2]).parse("""{"tBool":true}""") mustEqual (Success(Right(Child2(true))))
    }
  }

  "parse as object" should {
    val classParser = /("animalType", "dog" -> as[Canine], "cat" -> as[Feline])
