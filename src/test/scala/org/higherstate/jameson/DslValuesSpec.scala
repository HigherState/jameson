...
class DslValuesSpec extends WordSpec with MustMatchers {
  ...
  "Open Map Parser" should {
    ...
    "not fail if not required key value is not found" in {
      asMap("tFloat" -> as[Float] is required).parse(json).isSuccess mustEqual true
    }
    ...
    "supports required ? with default value" in {
      asMap("tFloat" -> getAsOrElse[Float](3.5F)).parse(json) mustEqual Success(map + ("tFloat" -> 3.5F))
    }
  ...
  "Either parser" should {
    ...
    "Succeed class right" in {
      val p = asEither[Child1, Child2].parse("""{"tBool":true}""") mustEqual(Success(Right(Child2(true))))
    }
  ...
  "function parser" should {
    "Succeed with a key into function" in {
      ...
      asMap("key" map a).parse("""{"key":3}""") mustEqual(Success(Map("key" -> "3")))
    }
    ...
    "Succeed with a list tuple function pipe" in {
      (as[Int,Long] map (_ + _)).parse("""[1,2]""") mustEqual Success(3)
    }
    "Succeed with a map tuple function pipe" in {
      (as("int" -> as[Int], "long" -> as[Long]) map (_ + _)).parse("""{"long":3,"int":7}""") mustEqual Success(10)
    }
    "Succeed with a map tuple function which requires a single tuple value" in {
      (as("int" -> as[Int], "long" -> as[Long]) map ((t:(Int, Long)) => t._1 + t._2)).parse("""{"long":3,"int":7}""") mustEqual Success(10)
    }
  }
  ...
  "tuple list parsing" should {
    "Succeed with simple 2 tuple" in {
      as[Int, String].parse("[567,\"test\"]") mustEqual Success((567,"test"))
    }
    "Succeed with nested 2 tuple" in {
      as(asMap[Any], asList[Any]).parse("""[{"key":"value"},[1,2,3,4]]""") mustEqual Success(Map("key" -> "value"), List(1,2,3,4))
    }
    "Succeed with simple 3 tuple" in {
      as[Int, String, Boolean].parse("[567,\"test\", true]") mustEqual Success((567,"test", true))
    }
    "Succeed with default tail" in {
      as(as[Int], getAs[String]).parse("[567]") mustEqual Success((567, None))
    }
    "Succeed with null value" in {
      as(getAsOrElse[Int](3), getAs[String]).parse("[null, \"test\"]") mustEqual Success((3, Some("test")))
    }
    "Succeed with extended defaults" in {
      as(as[Int], getAs[Boolean], getAs[String]).parse("[123]") mustEqual Success((123, None, None))
    }
  }

  "tuple map parser" should {
    "Succeed with a simple 2 tuple" in {
      as("int" -> as[Int], "string" -> as[String]).parse("""{"string":"s","int":1}""") mustEqual Success((1,"s"))
    }
    "Succeed with a simple 2 tuple with extra key pairs" in {
      as("int" -> as[Int], "string" -> as[String]).parse("""{"bool":true, "string":"s","list":[1,2,3,4],"int":1}""") mustEqual Success((1,"s"))
    }
    "Succeed with a simple 2 tuple with default value" in {
      as("int" -> getAs[Int], "string" -> as[String]).parse("""{"string":"s"}""") mustEqual Success((None, "s"))
    }
    "Succeed with simple 3 tuple" in {
      as("int" -> as[Int], "string" -> as[String], "bool" -> as[Boolean]).parse("""{"bool":false, "string":"s","int":1}""") mustEqual Success((1,"s", false))
    }
    "Succeed with a piped value" in {
      as("int" -> as[Int] map (_ + 5), "string" -> as[String]).parse("""{"string":"s","int":1}""") mustEqual Success((6,"s"))
    }
    "Succeed with a range of keys" in {
      as(("int"|"INT") -> as[Int], "string" -> as[String]).parse("""{"string":"s","INT":1}""") mustEqual Success((1,"s"))
    }
  }
  ...
  "nested parser " should {
    "handle nested buffering parser" in {
      val parser = as(as[Int], asEither(asMap("one" -> as[Int], "three" -> as[Boolean]), asMap("one" -> as[Int], "three" -> as[Int])), matchAs("type", "int" -> asMap("value" -> as[Int]), "either" -> asMap("value" -> asEither(asMap[Any], as[String])) is excludekeys), asMap("value" -> as[Any]))
