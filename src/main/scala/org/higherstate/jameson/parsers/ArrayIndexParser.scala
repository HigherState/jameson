...
case class ArrayIndexParser[T](index:Int, parser:Parser[T]) extends Parser[T] {
  ...
  private def iterate(tokenizer:Tokenizer, path: Path, index:Int):Valid[T] = tokenizer.head match {
    case ArrayEndToken =>
  ...
  private def iterateRemaining(tokenizer:Tokenizer, path:Path, value:T):Valid[T] = tokenizer.head match {
    case ArrayEndToken =>
      Success(value)
    case EndToken =>
      Failure(UnexpectedTokenFailure("Unexpected token.", EndToken, path))
